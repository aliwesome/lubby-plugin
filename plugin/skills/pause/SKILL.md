...
When pausing, also clear the user's current presence so they disappear from the dashboard immediately — temporarily unpause logic is not needed; just send a `cancelled` event *before* flipping the flag:
